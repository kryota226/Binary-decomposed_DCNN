...
def _verify(class_map, label_map, n_classes):
    ...
    assert class_map.shape == label_map.shape, '{} != {}'.format(class_map.shape, label_map.shape)
    assert np.max(class_map) < n_classes
    assert isinstance(class_map, np.ndarray), type(class_map)
    assert isinstance(label_map, np.ndarray), type(label_map)
...
def _generate_conf_matrix(class_map, label_map, n_classes=2):
    ...
    return np.asarray(conf_matrix, dtype=np.uint32)
