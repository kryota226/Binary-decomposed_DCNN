...
#pragma omp parallel for
        for(int d1 = 0; d1 < OutT::DIM_1; ++d1) {
            ...
            for(int basis = 0; basis < N_BASIS; ++basis) {
                double Mx = 0;
                const int index_M_base = BinaryMatrices::IndexT::indexOf(d1, basis, 0); 
                const std::uint64_t * M_base = &M_ui.data[index_M_base];
                for(int exp = 0; exp < QUANTIZE_BITS; ++exp) {
                    // 同じ復元係数を持つベクトルのハミング距離をとる
                    int accum_dist = 0;
                    const int index_src_base = BinaryInT::IndexT::indexOf(exp, 0); 
                    const std::uint64_t * src_base = &src_ui.data[index_src_base];
                    for(int dim_ui = 0; dim_ui < N_BITSET; ++ dim_ui) {
                        //std::uint64_t Mx_and = src_ui.at(exp, dim_ui) & M_ui.at(d1, basis, dim_ui);
                        std::uint64_t Mx_and = M_base[dim_ui] & src_base[dim_ui];
