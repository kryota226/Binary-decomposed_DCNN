...
template <
    typename InT,
    int N_OUT_MAPS, int N_ROWS_FILTER, int N_COLS_FILTER,
    int N_ROWS_STRIDE=1, int N_COLS_STRIDE=1,
    int N_ROWS_PAD=0, int N_COLS_PAD=0,
    int QUANTIZE_BITS=1,
    int N_BASIS=1
>
...
public:
    ...
    OutT forward(const InT & src)
    {
        ...
        // ********** 入力の最大ｰ最小値間を量子化 ********* //
        const double max_val = pad_src.max();
        const double min_val = pad_src.min();
...
#if 0
        OutT result;
        forward_body(result, restore_coeff, min_val, src_ui, x_norm);
#endif
        // ********** 内積計算 ********** //
        OutT result;
        for(int d1 = 0; d1 < OutT::DIM_1; ++d1) {
...
/*
    class ParallelLoopBody_ : public cv::ParallelLoopBody
    {
    private:
        int d1;
        const FastConvolution * p_conv;
        OutT * p_result;
        const RestoreCoeff * p_restore_coeff;
        const double min_val;
        const BinaryInT * p_src_ui;
        const Norm * p_x_norm;
        const double offset_;
    public:
        ParallelLoopBody_(
            const int d1_,
            const FastConvolution * p_conv_,
            OutT * p_result_,
            const RestoreCoeff * p_restore_coeff_,
            const double min_val_,
            const BinaryInT * p_src_ui_,
            const Norm * p_x_norm_,
            const double offset
            )
            : d1(d1_), p_conv(p_conv_),
            p_result(p_result_), p_restore_coeff(p_restore_coeff_), min_val(min_val_),
            p_src_ui(p_src_ui_), p_x_norm(p_x_norm_), offset_(offset)
        {}
        void operator()(const cv::Range & range) const {
            const auto & bias_ = p_conv->bias_;
            const auto & M_ui = p_conv->M_ui;
            const auto & c = p_conv->c;
            const auto & restore_coeff = *p_restore_coeff;
            const auto & src_ui = *p_src_ui;
            const auto & x_norm = *p_x_norm;
            for(int d2 = range.start; d2 < range.end; ++d2) {
                //auto * dst_base = &p_result->data[OutT::IndexT::indexOf(d1, d2, 0)];
                for(int d3 = 0; d3 < OutT::DIM_3; ++d3) {
                    double cMx = bias_.at(d1);
                    for(int k = 0; k < N_BASIS; ++k) {
                        double Mx = 0.0;
                        const std::uint64_t * wb_base = &M_ui.data[BinaryMatrices::IndexT::indexOf(d1, k, 0)];
                        const auto & x_norm_base = &x_norm.data[Norm::IndexT::indexOf(d2, d3, 0)];
                        for(int exp = 0; exp < QUANTIZE_BITS; ++exp) {
                            double accum_dist = 0.0;
                            const std::uint64_t * xb_base = &src_ui.data[BinaryInT::IndexT::indexOf(d2, d3, exp, 0)];
                            for(int dim_ui = 0; dim_ui < N_BITSET; ++dim_ui) {
                                const std::uint64_t & wb_ref = wb_base[dim_ui];
                                const std::uint64_t & xb_ref = xb_base[dim_ui];
                                accum_dist += _mm_popcnt_u64(wb_ref & xb_ref);
                            }
                            Mx += (2.0 * accum_dist - x_norm_base[exp]) * restore_coeff.at(exp);
                        }
                        cMx += c.at(d1, k) * Mx;
                    }
                    (*p_result).at(d1, d2, d3) = cMx + offset_;
                    //dst_base[d3] = cMx + offset_;
                }
            }
        }
    };
    ...
    forward_body(
        OutT & result,
        const RestoreCoeff & restore_coeff,
        const double min_val,
        const BinaryInT & src_ui,
        const Norm & x_norm)
    {
        for(int d1 = 0; d1 < OutT::DIM_1; ++d1) {
            const double offset_ = offset.at(d1) * min_val;
            cv::parallel_for_(
                cv::Range(0, OutT::DIM_2),
                ParallelLoopBody_(d1, this, &result, &restore_coeff,
                    min_val, &src_ui, &x_norm, offset_)
                );
        }
    }
*/
